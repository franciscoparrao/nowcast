...
dependency), PS-InSAR/SBAS deformation, and a wildfire-spread model with a
post-fire susceptibility cascade; a command-line runner exposes the engine
(run, backtest, explain) and PyO3 bindings expose it to Python. The codebase is covered by
$\sim$58 unit and documentation tests, the linter runs clean under
\texttt{-D warnings}, and twenty-four runnable examples reproduce every result below.
...
maximising Kling--Gupta efficiency) and evaluated out of sample on 2003--2016; it
...
per cell (wrapping a distributed snowmelt model); an InSAR provider turns a
...
debris-flow hazard the scar is known to carry. Each is a runnable example. Their
...
is a bounded \emph{index}; the engine ships isotonic calibration that maps it to a
probability with reliability diagnostics (Brier score and skill, Wilson intervals,
...
PyO3 Python bindings, plus twenty-four examples) and all
